...
import com.parrot.freeflight.activities.DroneHeadTracker;
        ...
        DroneBatteryChangedReceiverDelegate, DroneFlyingStateReceiverDelegate, DroneCameraReadyActionReceiverDelegate, DroneRecordReadyActionReceiverDelegate, SettingsDialogDelegate
{
    ...
    private DroneHeadTracker tracker;
...
//
//        //  google TV requires specific sensor manager and device rotation
//        if (this.isGoogleTV) {
...
//            deviceOrientationManager = new DeviceOrientationManager(new DeviceSensorManagerWrapper(this), this);
        ...
        // TODO: 4/5/16 head tracker initialized
        tracker = DroneHeadTracker.createTrackerFromContext(this.getApplicationContext());
...
//        } else {  TODO//basically boils down to "do this if not under gps control so leaving it in for now just in case"
...
//            }
//        }
        //TODO//all vectors are calculated in DroneHeadTransform.geteulers()
        float[] values = new float[4]; //{pitch,roll,yaw,gaz}
                values = tracker.getEulers(values);

        // TODO: 4/5/16 set and send drone vectors
        droneControlService.setPitch(values[0]);
        droneControlService.setRoll(values[1]);
        droneControlService.setYaw(values[2]);
        droneControlService.setGaz(values[3]);
